...
MostPopular <- function(my.year) {
  return(paste("The most popular name of",my.year,"was",name[year == my.year][prop[year == my.year] == max(prop[year == my.year])]))
}
...
HowPopular <- function(spec.name, spec.year) {
  return(paste(spec.name,"was",prop[name == spec.name && year == spec.year])
}
